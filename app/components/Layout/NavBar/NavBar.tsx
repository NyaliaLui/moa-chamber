...
const StyledNavLink = ({
  ...
  <NavLink
    className={`${className ?? ''} ${
      isActive ? 'text-blue-site' : 'hover:text-blue-site'
    }`}
    {...linkProps}
  />
);
  ...
  return (
    <>
      ...
      <button
        ...
        onClick={toggleOpen}
      >
        <div className="space-y-2 absolute top-0 right-0 bg-blue-site p-3">
          {(isMenuShown
            ? [
      ...
      >
        <ul className="flex flex-col items-center md:flex-row gap-10 md:gap-4 min-[900px]:gap-5 lg:gap-12 justify-end text-sm md:text-[15px] leading-[22px]">
          {navbarItems
            .filter((o) => o.ref !== '/join' && !o.ref.includes('paypal.com'))
            .map(({ ref, label }) => (
          ...
          {navbarItems
            .filter((o) => o.ref === '/join' || o.ref.includes('paypal.com'))
            .map(({ ref, label, color, outline }) => (
              <li key={ref} className="relative">
                <Button color={color} size="sm" href={ref} outline={outline}>
                  {label}
                </Button>
              </li>
            ))}
